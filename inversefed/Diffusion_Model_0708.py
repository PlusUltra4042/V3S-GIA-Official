...
class Diffusion:
    ...
    def sample(self, n):#, enable_grad=False, start_noise
        logging.info(f"Sampling {n} new images....")
        # if not enable_grad:
        #     self.model.eval()  # 推理模式关闭随机层
        #     print("eval")
        # else:
        #     self.model.train()  # 训练模式启用随机层（可选）
        #     print("train")
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            # for i in reversed(range(1, self.noise_steps)):
                ...
                for i in range(n):
                    ...
                    re_image = predicted_noise[i].permute(1, 2, 0)  # 转为 [H, W, C]
                    plt.imshow(re_image.detach().cpu().numpy())
                    plt.title(f"Recovered_predicted_noise")
                    plt.axis('off')
                plt.show()
